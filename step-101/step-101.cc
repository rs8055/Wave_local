...
namespace Step101
{
  ...
  template <int dim>
  double WaveSpeed<dim>::value(const Point<dim>  &p,
  ...
  template <int dim>
  double InterfaceBoundaryValues<dim>::value(const Point<dim> &p,
                                    const unsigned int component) const
  {
    AssertIndexRange(component, this->n_components);
    (void)component;
  ...
  void WaveSolver<dim>::solve(const double evaluating_time, const LinearAlgebra::distributed::Vector<double> previous_solution, LinearAlgebra::distributed::Vector<double> &solution_out)
  {
    ...
    // const double nitsche_parameter = 5;
    ...
    for (unsigned int cycle = 0; cycle <= n_refinements; cycle++)
      {
        ...
        convergence_table.add_value("Final Time", time);
